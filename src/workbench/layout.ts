...
import { $, addDisposableListener, Dimension, getClientArea } from '../base/browser/dom.js';
...
export class Layout extends Disposable {
  ...
  registerPart(location: PartLocation, part: Part): void {
    ...
    switch (location) {
      case 'titlebar':
        part.create(this._titleBarContainer);
        break;
      case 'activitybar':
      case 'sidebar':
      case 'editor':
        ...
        break;
      case 'panel':
        part.create(this._panelContainer);
        break;
      case 'statusbar':
        part.create(this._statusBarContainer);
        break;
    }
  }
  ...
  private _layoutParts(): void {
    ...
    // Activity bar and sidebar take from the main area
    const activitybarWidth = activitybar ? 48 : 0;
    const sidebarWidth = sidebar ? Math.max(170, rootSize.width * 0.2) : 0;
